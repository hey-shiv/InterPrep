...
export function GlowButton({
  ...
  const baseClass = 'inline-flex items-center justify-center gap-2 font-semibold rounded-[10px] transition-all duration-200 ease-out cursor-pointer select-none'

  const variantClass = {
    primary: isDisabled
      ? 'bg-bg3 text-t4 cursor-not-allowed'
      : 'bg-accent text-white shadow-glow-sm hover:shadow-glow-md',
    secondary: 'bg-transparent border border-border text-t1 hover:border-accent hover:text-accent hover:bg-[rgba(99,102,241,0.05)]',
